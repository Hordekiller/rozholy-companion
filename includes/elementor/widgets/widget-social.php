...
class Rozholy_Widget_Stats_Counter extends \Elementor\Widget_Base {
	...
	public function get_categories(): array {
		return array( 'rozholy-salon' ); }
	public function get_keywords(): array {
		return array( 'stats', 'counter', 'statistics' ); }

	protected function register_controls() {
		$this->start_controls_section(
			'content_section',
			array(
				'label' => esc_html__( 'Stats', 'rozholy-companion' ),
				'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
			)
		);
		$repeater = new \Elementor\Repeater();
		...
		$repeater->add_control(
			...
			array(
				...
				'type'    => \Elementor\Controls_Manager::TEXT,
				'default' => esc_html__( 'Happy Clients', 'rozholy-companion' ),
			)
		);
		$this->add_control(
			'items',
			array(
				...
				'default'     => array(
					array(
						'number' => 15,
						'suffix' => '+',
						'label'  => esc_html__( 'Years Experience', 'rozholy-companion' ),
					),
					array(
						'number' => 5000,
						'suffix' => '+',
						'label'  => esc_html__( 'Happy Clients', 'rozholy-companion' ),
					),
					array(
						'number' => 50,
						'suffix' => '+',
						'label'  => esc_html__( 'Expert Staff', 'rozholy-companion' ),
					),
					array(
						'number' => 100,
						'suffix' => '%',
						'label'  => esc_html__( 'Satisfaction', 'rozholy-companion' ),
					),
				),
				'title_field' => '{{{ label }}}',
			)
	...
	public function render(): void {
		...
		?>
		<div style="display:grid;grid-template-columns:repeat(<?php echo intval( $s['columns'] ?: 4 ); ?>,1fr);gap:24px;text-align:center;">
			<?php foreach ( $items as $item ) : ?>
				<div><div class="rz-counter" style="font-size:clamp(2rem,4vw,3.5rem);font-weight:700;background:linear-gradient(135deg,#d4a0a0,#b8a0c0);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;"><?php echo intval( $item['number'] ); ?><?php echo esc_html( $item['suffix'] ?: '' ); ?></div><p style="color:#7a7a7a;font-size:.95rem;margin:4px 0 0;"><?php echo esc_html( $item['label'] ); ?></p></div>
			<?php endforeach; ?>
		</div>
		<?php
	}
