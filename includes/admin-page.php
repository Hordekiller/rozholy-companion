...
function rozholy_companion_admin_assets( $hook ) {
	$admin_pages = array( 'toplevel_page_rozholy-dashboard', 'rz_booking_page_rz-booking-manager' );
	if ( ! in_array( $hook, $admin_pages, true ) ) {
		return;
	}

	wp_enqueue_style( 'wp-components' );
	wp_enqueue_style( 'rozholy-companion-admin', ROZHOLY_COMPANION_URI . 'build/admin.css', array( 'wp-components' ), ROZHOLY_COMPANION_VERSION );

	$asset_file = ROZHOLY_COMPANION_DIR . 'build/admin.asset.php';
	$deps       = array( 'wp-components', 'wp-element', 'wp-i18n', 'wp-api-fetch', 'wp-data', 'wp-notices', 'wp-hooks' );
